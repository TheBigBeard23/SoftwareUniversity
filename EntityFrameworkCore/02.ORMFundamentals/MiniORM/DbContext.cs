...
public class DbContext
{
    ...
    private bool IsObjectValid(object e)
    {
        ...
        bool validationResult = Validator.TryValidateObject(e, validationContext, errors, true);
    ...
    {
        foreach (KeyValuePair<Type, PropertyInfo> dbSetInfo in dbSetProperties)
        {
            Type dbSetEntityType = dbSetInfo.Key;
            object dbSetInstance = dbSetInfo.Value.GetValue(this);
            MethodInfo mapRealationsMethod = typeof(DbContext)
                .GetMethod("MapRelations", BindingFlags.Instance | BindingFlags.NonPublic)
                .MakeGenericMethod(dbSetEntityType);

            mapRealationsMethod.Invoke(this, new object[] { dbSetInstance });
        }
    }
        ...
        where TCollection : class, new()
    {
        ...
        PropertyInfo[] collectionTypePrimaryKey = collectionType
            .GetProperties()
            .Where(pi => pi.HasAttribute<KeyAttribute>())
            .ToArray();

        PropertyInfo primaryKey = collectionTypePrimaryKey.First();
        PropertyInfo foreignKey = entityType
            .GetProperties()
            .First(pi => pi.HasAttribute<KeyAttribute>());

        bool isManyToMany = collectionTypePrimaryKey.Length >= 2;
    ...
    private void MapNavigationProperty<TEntity>(DbSet<TEntity> dbSet) 
        where TEntity : class, new()
    {
        Type entityType = typeof(TEntity);
